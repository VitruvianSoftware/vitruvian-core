...
export class Network extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.network = new gcp.compute.Network(
      `${name}-network`,
      {
        ...
        deleteDefaultRoutesOnCreate:
          args.deleteDefaultInternetGatewayRoutes ?? false,
    ...
    for (const subnet of args.subnets) {
      const subnetResource = new gcp.compute.Subnetwork(
        `${name}-${subnet.subnetName}`,
        {
          ...
          logConfig: subnet.subnetFlowLogs
            ? {
                aggregationInterval:
                  subnet.subnetFlowLogsInterval ?? "INTERVAL_5_SEC",
                flowSampling: subnet.subnetFlowLogsSampling ?? 0.5,
                metadata:
                  subnet.subnetFlowLogsMetadata ?? "INCLUDE_ALL_METADATA",
    ...
    if (args.sharedVpcHost) {
      new gcp.compute.SharedVPCHostProject(
        `${name}-svpc-host`,
        {
          project: args.projectId,
        },
    ...
    for (const rule of args.rules) {
      new gcp.compute.Firewall(
        `${name}-${rule.name}`,
        {
          ...
          destinationRanges:
            rule.direction === "EGRESS" ? rule.ranges : undefined,
