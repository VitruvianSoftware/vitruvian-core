...
export class CAIMonitoring extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    const rolesToMonitor =
      args.rolesToMonitor && args.rolesToMonitor.length > 0
        ? args.rolesToMonitor
    ...
    // ====================================================================
    const caiSA = new gcp.serviceaccount.Account(
      `${name}-sa`,
      {
    ...
    const findingsEditorIAM = new gcp.organizations.IAMMember(
      `${name}-sa-findings-editor`,
      {
        orgId: args.orgId,
    ...
    // ====================================================================
    const caiTopic = new gcp.pubsub.Topic(
      `${name}-topic`,
      {
        project: args.projectId,
    ...
    const caiFeed = new gcp.cloudasset.OrganizationFeed(
      `${name}-org-feed`,
      {
    ...
    // ====================================================================
    const sccSource = new gcp.securitycenter.V2OrganizationSource(
      `${name}-scc-source`,
      {
    ...
    new gcp.cloudfunctionsv2.Function(
      `${name}-function`,
      {
        project: args.projectId,
        location,
        name: "caiMonitoring",
        description:
          "Check on the Organization for members (users, groups and service accounts) that contains the IAM roles listed.",
        buildConfig: {
          runtime: "nodejs20",
