...
export class VpnHa extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.gateway = new gcp.compute.HaVpnGateway(
      `${name}-gateway`,
      {
        name: args.name,
    ...
    if (args.routerAdvertiseConfig) {
      ...
      if (args.routerAdvertiseConfig.ipRanges) {
        bgpConfig.advertisedIpRanges = args.routerAdvertiseConfig.ipRanges.map(
          (r) => ({
            range: r.range,
    ...
    if (args.peerExternalGateway) {
      peerGateway = new gcp.compute.ExternalVpnGateway(
        `${name}-peer-gw`,
        {
          name: args.peerExternalGateway.name,
          project: args.projectId,
          redundancyType:
            args.peerExternalGateway.redundancyType ?? "TWO_IPS_REDUNDANCY",
    ...
    for (let i = 0; i < args.tunnels.length; i++) {
      ...
      const tunnel = new gcp.compute.VPNTunnel(
        `${name}-tunnel-${i}`,
        {
          ...
          peerExternalGatewayInterface: peerGateway
            ? tunnelCfg.vpnGatewayInterface
            : undefined,
      ...
      const iface = new gcp.compute.RouterInterface(
        `${name}-iface-${i}`,
        {
