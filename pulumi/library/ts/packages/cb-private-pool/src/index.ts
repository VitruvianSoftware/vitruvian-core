...
export class CbPrivatePool extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    if (pwp.createPeeredNetwork) {
      ...
      const peeredNetwork = new Network(
        `${name}-peered-network`,
        {
    ...
    if (pwp.enableNetworkPeering && networkId) {
      const workerPoolRange = new gcp.compute.GlobalAddress(
        `${name}-worker-range`,
        {
      ...
      const conn = new gcp.servicenetworking.Connection(
        `${name}-worker-conn`,
        {
          network: networkId,
      ...
      new gcp.compute.NetworkPeeringRoutesConfig(
        `${name}-peering-routes`,
        {
          project: args.projectId,
          ...
          rules: [
            {
              ...
              description:
                "allow ingress from the IPs configured for service networking",
              direction: "INGRESS",
              priority: 100,
    ...
    const pool = new gcp.cloudbuild.WorkerPool(
      `${name}-worker-pool`,
      {
