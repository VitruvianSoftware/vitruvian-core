  ...
  constructor(
    ...
    if (sourceType === "CSR") {
      for (const repoName of args.sourceRepos ?? []) {
        this.sourceRepos[repoName] = new gcp.sourcerepo.Repository(
          `${name}-csr-${repoName}`,
          {
    ...
    this.artifactRegistry = new gcp.artifactregistry.Repository(
      `${name}-ar`,
      {
        project: args.projectId,
        repositoryId: arId,
        ...
        format: "DOCKER",
        description:
          "Builder images for Cloud Build pipelines. Managed by Pulumi.",
      },
      { parent: this },
    );
    ...
    for (const [key, cfg] of Object.entries(args.triggers)) {
      ...
      this.planTriggers[key] = new gcp.cloudbuild.Trigger(
        `${name}-plan-${key}`,
        planArgs,
        { parent: this },
      );
      this.applyTriggers[key] = new gcp.cloudbuild.Trigger(
        `${name}-apply-${key}`,
        applyArgs,
        { parent: this },
      );
