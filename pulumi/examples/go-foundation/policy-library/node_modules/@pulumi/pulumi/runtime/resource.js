...
function registerResource(res, parent, t, name, custom, remote, newDependency, props, opts, sourcePosition, stackTrace, packageRef) {
    ...
    debuggable_1.debuggablePromise(resopAsync
        ...
        if (resop.replacementTrigger !== undefined) {
            const options = {};
            if (output_1.Output.isInstance(resop.replacementTrigger)) {
                const isKnown = await resop.replacementTrigger.isKnown;
                options.keepOutputValues = !isKnown || settings_1.isDryRun();
            }
            const serializedTrigger = await rpc_1.serializeProperty(`${label}.replacementTrigger`, resop.replacementTrigger, new Set(), options);
        ...
        if (replaceWithURNs.length > 0) {
            if (!state_1.getStore().supportsReplaceWith) {
                throw new Error("The Pulumi CLI does not support the ReplaceWith option. Please update the Pulumi CLI.");
            }
...
async function prepareResource(label, res, parent, custom, remote, props, opts, type, name) {
    ...
    if (remote || custom || opts.urn !== undefined) {
        resolvers = rpc_1.transferProperties(res, label, props);
    }
    /** IMPORTANT!  We should never await prior to this line, otherwise the Resource will be partly uninitialized. */
    // Before we can proceed, all our dependencies must be finished.
    const replaceWithDependencies = await dependsOn_1.gatherExplicitDependencies(opts.replaceWith);
    ...
    const replaceWithResources = replaceWithDependencies.length > 0 ? Array.from(new Set(replaceWithDependencies)) : undefined;
    return {
...
class StubErrorHook {
    ...
    static isInstance(obj) {
        return utils.isInstance(obj, "__pulumiErrorHook");
    }
}
exports.StubErrorHook = StubErrorHook;
/**
...
async function registerErrorHook(hook) {
    if (!state_1.getStore().supportsErrorHooks) {
...
exports.registerErrorHook = registerErrorHook;
