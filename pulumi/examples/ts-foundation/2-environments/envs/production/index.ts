/**
 * 2-environments/envs/production/index.ts
 * Mirrors: 2-environments/envs/production/main.tf
 */
...
export = async () => {
  ...
  const result = deployEnvBaseline({
    ...
    assuredWorkloadConfiguration: config.getObject<any>(
      "assured_workload_configuration",
    ),
  });

  // Exports — matches upstream TF 2-environments/envs/production/outputs.tf exactly
