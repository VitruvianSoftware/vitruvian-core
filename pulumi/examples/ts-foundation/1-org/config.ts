...
export function loadOrgConfig(config: pulumi.Config): OrgConfig {
  ...
  const parentType =
    parentFolder !== "" ? ("folder" as const) : ("organization" as const);
  const parentId = parentFolder !== "" ? parentFolder : orgId;

  return {
    orgId,
    billingAccount: config.require("billing_account"),
    enableBillingAccountSink:
      config.getBoolean("enable_billing_account_sink") ?? true,
    ...
    enableHubAndSpokeTransitivity:
      config.getBoolean("enable_hub_and_spoke_transitivity") ?? false,
    domainsToAllow: config.getObject<string[]>("domains_to_allow") || [],
    essentialContactsDomainsToAllow:
      config.getObject<string[]>("essential_contacts_domains_to_allow") || [],
    essentialContactsLanguage:
      config.get("essential_contacts_language") || "en",
    sccNotificationName: config.get("scc_notification_name") || "scc-notify",
    sccNotificationFilter:
    ...
    createAccessContextManagerAccessPolicy:
      config.getBoolean("create_access_context_manager_access_policy") ?? true,
    enforceAllowedWorkerPools:
      config.getBoolean("enforce_allowed_worker_pools") ?? false,
    cloudBuildPrivateWorkerPoolId:
      config.get("cloud_build_private_worker_pool_id") || undefined,
    caiMonitoringKmsForceDestroy:
      config.getBoolean("cai_monitoring_kms_force_destroy") ?? false,
    projectDeletionPolicy: config.get("project_deletion_policy") || "PREVENT",
    folderDeletionProtection:
      config.getBoolean("folder_deletion_protection") ?? true,
    logExportStorageForceDestroy:
      config.getBoolean("log_export_storage_force_destroy") ?? false,
    logExportStorageVersioning:
      config.getBoolean("log_export_storage_versioning") ?? false,
    logExportStorageLocation:
      config.get("log_export_storage_location") || undefined,
    logExportStorageRetentionPolicy:
      config.getObject("log_export_storage_retention_policy") || undefined,
    billingExportDatasetLocation:
      config.get("billing_export_dataset_location") || undefined,
    remoteStateBucket: config.require("remote_state_bucket"),
    requiredGroups:
      config.getObject<Record<string, string>>("required_groups") || {},
    optionalGroups:
      config.getObject<Record<string, string>>("optional_groups") || {},
    projectBudget:
      config.getObject<Record<string, any>>("project_budget") || {},
