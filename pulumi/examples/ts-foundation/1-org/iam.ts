...
export function deployOrgIAM(cfg: OrgConfig, proj: OrgProjectRefs): void {
  ...
  if (securityReviewer) {
    orgOrFolderIAMMember(
      "security-reviewer",
      cfg,
      "roles/iam.securityReviewer",
      `group:${securityReviewer}`,
    );
  }
  ...
  if (cfg.enableKmsKeyUsageTracking) {
    const kmsOrgIdentity = new command.local.Command(
      "kms-org-service-identity",
      {
  ...
  // ========================================================================
  if (
    cfg.enableHubAndSpoke &&
    cfg.networksStepTerraformSaEmail &&
    proj.netHubProjectId
  ) {
  ...
  if (cfg.enableSccResources) {
    const caiBuilderSa = new gcp.serviceaccount.Account(
      "cai-monitoring-builder",
      {
        project: proj.sccProjectId,
        accountId: "cai-monitoring-builder",
        description:
          "Service account for Cloud Build to provision CAI monitoring Cloud Functions",
