...
export = async () => {
    ...
    const template = new InstanceTemplate("example-template", {
        ...
        diskSizeGb: 100,
        diskType: "pd-ssd",
        subnetwork: `projects/${config.get("shared_vpc_host_project") || ""}/regions/${region}/subnetworks/sb-p-svpc-${region}`,
        serviceAccount: {
            email: sa.email,
            scopes: ["compute-rw"],
        },
    ...
    new gcp.serviceaccount.IAMMember("p-workload-identity-binding", {
