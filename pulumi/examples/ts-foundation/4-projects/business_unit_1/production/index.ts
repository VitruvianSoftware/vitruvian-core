...
export = async () => {
    ...
    const sharedVpcHostProjectId = orgRef.getOutput("shared_vpc_projects").apply(
        (p: Record<string, { project_id: string }>) => p["production"]?.project_id || ""
    );
    ...
    new gcp.accesscontextmanager.ServicePerimeterResource(`production-confidential-space-perimeter-attachment`, {
    ...
    const workloadIdentitySa = new gcp.serviceaccount.Account(`production-confidential-space-sa`, {
    ...
    new gcp.storage.Bucket(`production-cmek-bucket`, {
