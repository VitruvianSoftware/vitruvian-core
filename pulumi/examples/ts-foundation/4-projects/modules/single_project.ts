...
export function deploySingleProject(
  name: string,
  args: SingleProjectArgs,
): SingleProjectOutputs {
  ...
  const enabledApis = [
    ...new Set([...(args.activateApis ?? []), "billingbudgets.googleapis.com"]),
  ];
  ...
  if (
    args.enableCloudbuildDeploy &&
    args.appInfraPipelineServiceAccounts &&
    args.saRoles
  ) {
      ...
      if (sa) {
        for (const role of roles) {
          new gcp.projects.IAMMember(
            `${name}-pipeline-${repo}-${role.replace(/\//g, "-")}`,
            {
