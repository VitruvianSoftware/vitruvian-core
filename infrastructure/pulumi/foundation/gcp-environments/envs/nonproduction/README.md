# 2-environments/envs/nonproduction

Thin env root for the **nonproduction** environment — the Pulumi/Go port of upstream
[terraform-example-foundation `2-environments/envs/nonproduction`](https://github.com/terraform-google-modules/terraform-example-foundation/tree/master/2-environments/envs/nonproduction).
...
- `fldr-nonproduction` — the environment folder, with the `environment_nonproduction`
  tag value from 1-org bound to it
- `prj-n-kms` — the environment-level Cloud KMS project
- `prj-n-secrets` — the environment-level Secret Manager project
...
| `Pulumi.production.yaml` | `terraform.tfvars` | checked-in stack config for the live org |
...
Stack config is checked in (`Pulumi.production.yaml`, stack `production`,
backend Pulumi Cloud `ipv1337`). Preview / deploy via the Bazel Pulumi wrapper:

```sh
bazel run //infrastructure/pulumi/foundation/gcp-environments/envs/nonproduction:preview
bazel run //infrastructure/pulumi/foundation/gcp-environments/envs/nonproduction:up
```

Promotion order across the env leaves: development → nonproduction → production.
