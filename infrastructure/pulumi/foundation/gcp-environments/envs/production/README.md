# 2-environments/envs/production

Thin env root for the **production** environment — the Pulumi/Go port of upstream
[terraform-example-foundation `2-environments/envs/production`](https://github.com/terraform-google-modules/terraform-example-foundation/tree/master/2-environments/envs/production).
...
- `fldr-production` — the environment folder, with the `environment_production`
  tag value from 1-org bound to it
- `prj-p-kms` — the environment-level Cloud KMS project
- `prj-p-secrets` — the environment-level Secret Manager project
...
```sh
bazel run //infrastructure/pulumi/foundation/gcp-environments/envs/production:preview
bazel run //infrastructure/pulumi/foundation/gcp-environments/envs/production:up
```
