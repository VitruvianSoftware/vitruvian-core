# gcp-org / envs / shared (1-org)

The live organization stage root for the vitruviansoftware.dev org (Pulumi Go project `foundation-org`), mirroring upstream `terraform-example-foundation/1-org/envs/shared`. Each concern lives in its own file matching the upstream `.tf` file names:
...
Engine adaptations (kept, per the porting rules): backend/providers/versions live in `Pulumi.yaml`/`go.mod` instead of `backend.tf`/`providers.tf`/`versions.tf`; tfvars map to `Pulumi.production.yaml`. This live tree additionally carries the cold-deploy fixes (`api_propagation_seconds`/`ApisReady` gating, the pre-created CAI builder SA, module `Dependencies` threading) so a fresh apply is race-free.
...
| oss\_public\_invoker\_projects | Comma-separated project ids that receive a project-scoped AllowAll override of `iam.allowedPolicyMemberDomains` so `allUsers` can be granted `run.invoker` on the OSS Cloud Run services (live-only, data-driven DRS exception). | `string` (CSV) | `""` | no |
...
| enable\_scc\_resources | Create SCC resources (notification, CAI monitoring). SCC must be activated first (upstream `enable_scc_resources_in_terraform`). | `bool` | `false` | no |
| create\_access\_context\_manager\_policy | Whether to create the org-level Access Context Manager policy (upstream `create_access_context_manager_access_policy`). | `bool` | `true` | no |
...
| api\_propagation\_seconds | Cold-deploy propagation wait applied after a project's APIs are enabled before dependent resources are created (0 disables; live-only). | `int` | `120` | no |
