...
```go
import "foundation-org/modules/cai_monitoring"
...
caiOutputs, err := cai_monitoring.New(ctx, "cai-monitoring", &cai_monitoring.Args{
    ...
    Dependencies:  []pulumi.Resource{proj.SCCApisReady},
})
```
...
| BuilderSA | The pre-created `cai-monitoring-builder` service account used as the Cloud Build SA. Passing the resource (not a string-built email) makes the SA a data-driven dependency edge, so a cold deploy cannot reference the SA before it exists. | `*serviceaccount.Account` | yes |
| Dependencies | Gates every child of the library component on external resources (e.g. the SCC project's `ApisReady`) — required on a cold deploy because freshly-enabled APIs are not immediately usable. | `[]pulumi.Resource` | no |
...
- The Cloud Build service account (`cai-monitoring-builder`, upstream `build_service_account`) is created in `envs/shared/sa.go` and passed in as `BuilderSA`.
