...
> **Dormant catalog archetype.** No leaf calls `DeployConfidentialSpace` in the
> live foundation yet — like `env_base` and `serverless_space`, it is part of
> the stage-5 archetype catalog awaiting a consumer. It is an **upstream**
> module, so having it here gives the live catalog full parity with upstream
> `5-app-infra` (`env_base` + `confidential_space` + our `serverless_space`). It
> is kept compiling and unit-tested (`main_test.go`, mock-driven) so it is ready
> to instantiate the moment an environment needs a Confidential Space workload.
