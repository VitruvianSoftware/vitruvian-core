...
> Directory/Bazel target is `lab-gmail`; the Pulumi project (in Pulumi Cloud) remains `pulumi_lab_gmail`.
...
Prerequisites:
* [Pulumi CLI](https://www.pulumi.com/docs/install/) + Go (Pulumi compiles the program) — `bazel run //infrastructure/pulumi/accounts/personal:setup` verifies both
...
`bazel run //infrastructure/pulumi/accounts/personal:<verb> -- <args>`.

### 1. Bootstrap (login + stack)
```bash
bazel run //infrastructure/pulumi/accounts/personal:setup
```

### 2. Configure (minimal)
```bash
# Optional custom domain (creates a DomainMapping)
bazel run //infrastructure/pulumi/accounts/personal:config -- set customDomain hello.run.ipv1337.dev

# Optional Cloudflare automation (choose one)
bazel run //infrastructure/pulumi/accounts/personal:config -- set cloudflare:zoneId <ZONE_ID>
# OR
bazel run //infrastructure/pulumi/accounts/personal:config -- set cloudflare:zoneName ipv1337.dev

# Cloudflare token (stored encrypted)
bazel run //infrastructure/pulumi/accounts/personal:config -- set --secret cloudflare:apiToken <TOKEN>

# Recommended: keep DNS unproxied for certificate issuance
bazel run //infrastructure/pulumi/accounts/personal:config -- set cloudflare:proxied false
```

### 3. Deploy
```bash
bazel run //infrastructure/pulumi/accounts/personal:preview -- --diff   # review first
bazel run //infrastructure/pulumi/accounts/personal:up
```
...
## Removing Resources
```bash
bazel run //infrastructure/pulumi/accounts/personal:destroy
```
