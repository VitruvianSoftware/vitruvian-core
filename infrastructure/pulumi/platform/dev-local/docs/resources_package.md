...
The `resources` package (`github.com/VitruvianSoftware/vitruvian-core/infrastructure/pulumi/platform/dev-local/pkg/resources`) provides a set of high-level abstractions for creating and managing Kubernetes resources in a consistent, reusable manner. It serves as an abstraction layer between the raw Pulumi Kubernetes SDK and our application-specific code.
