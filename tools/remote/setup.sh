...
# LATENT-FOOTGUN GUARD: this template regenerates a COMMITTED file. It drifted
# once (older RBE image without the git>=2.28 fix; no remotecache profile) and
# a routine setup run would have silently downgraded CI's RBE config. Only
# rewrite when the endpoint actually differs from what is committed; keep the
# template in lockstep with tools/remote.bazelrc when either changes.
if [ -f tools/remote.bazelrc ] \
   && grep -q -- "--remote_executor=$ENDPOINT" tools/remote.bazelrc; then
  echo "✓ tools/remote.bazelrc already targets $ENDPOINT (left untouched)."
else
  mkdir -p tools
  {
    ...
    echo "# Default Linux execution platform/container for RBE actions. The image must ship"
    echo "# a modern glibc (>=2.28) for current Node/Python toolchains AND a modern git"
    ...
    echo ""
    echo "# Cache-only profile (--config=remotecache): share the BuildBuddy remote cache +"
    echo "# BES but WITHOUT remote execution — outputs stay local, actions run locally."
    echo "# Remote-cache I/O errors fall back to local execution; BES upload is fully"
    echo "# async so it never blocks or fails the build."
    echo "common:remotecache --remote_cache=$ENDPOINT"
    echo "common:remotecache --bes_backend=$ENDPOINT"
    [ -n "$BES_RESULTS" ] && echo "common:remotecache --bes_results_url=$BES_RESULTS"
    echo "common:remotecache --bes_upload_mode=fully_async"
    echo "common:remotecache --remote_timeout=10m"
