...
resolve_stable_revision() {
  ...
  if echo "$stderr_content" | grep -qiE "not_found|not found|cannot find|404|does not exist|may not exist"; then
...
ensure_stack() {
  ...
  pulumi_wrap stack select "$ENV" >/dev/null 2>&1 || \
    pulumi_wrap stack select "$ENV" -c >/dev/null 2>&1 || true
}
