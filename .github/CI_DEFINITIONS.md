...
| **Pipeline gates** | The repo-level Actions variables (`REPO_CONFIG_AUTO_APPLY`, `SYNC_AUTH_AUTO_APPLY`, `PULUMI_PREVIEW_ENABLED`, ...) that switch IaC workflows between advisory and applying. Pulumi-managed in `repo-config` (`pipelineGates`) — never hand-set. |
...
| **Required status check** | A CI job that _must_ pass before a PR can merge. Configured in branch protection rules (managed as IaC in `infrastructure/pulumi/platform/repo-config/`). |
