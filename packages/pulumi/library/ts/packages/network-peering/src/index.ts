...
export class NetworkPeering extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.localPeering = new gcp.compute.NetworkPeering(
      `${name}-local`,
      {
        name: `${prefix}-local`,
        ...
        exportSubnetRoutesWithPublicIp:
          args.exportSubnetRoutesWithPublicIp ?? true,
        importSubnetRoutesWithPublicIp:
          args.importSubnetRoutesWithPublicIp ?? false,
      },
      { parent: this },
    );

    this.peerPeering = new gcp.compute.NetworkPeering(
      `${name}-peer`,
      {
        name: `${prefix}-peer`,
