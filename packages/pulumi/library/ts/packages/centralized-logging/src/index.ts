...
export class CentralizedLogging extends pulumi.ComponentResource {
  ...
  private createSink(
    ...
    if (args.folderId) {
      return new gcp.logging.FolderSink(
        name,
        {
      ...
      return new gcp.logging.OrganizationSink(
        name,
        {
          orgId: args.orgId,
          name: args.sinkName,
  ...
  constructor(
    ...
    if (args.loggingBucketOptions?.name) {
      const bucket = new gcp.logging.ProjectBucketConfig(
        `${name}-log-bucket`,
        {
      ...
      if (args.loggingBucketOptions.linkedDatasetId) {
        const linkedDataset = new gcp.logging.LinkedDataset(
          `${name}-linked-dataset`,
          {
      ...
      const sink = this.createSink(
        `${name}-log-bucket-sink`,
        {
          orgId: args.orgId,
          folderId: args.folderId,
          sinkName:
            args.loggingBucketOptions.loggingSinkName ??
            `sk-c-logging-${args.loggingBucketOptions.name}`,
      ...
      const iam = new gcp.projects.IAMMember(
        `${name}-log-bucket-iam`,
        {
          project: args.projectId,
      ...
      if (args.billingAccount) {
        ...
        const billSink = new gcp.logging.BillingAccountSink(
          `${name}-log-bucket-billing-sink`,
          {
            name: billSinkName,
        ...
        const billIam = new gcp.projects.IAMMember(
          `${name}-log-bucket-billing-iam`,
          {
            project: args.projectId,
            role: "roles/logging.bucketWriter",
    ...
    if (args.bigqueryOptions?.datasetName) {
      const dataset = new gcp.bigquery.Dataset(
        `${name}-bq-dataset`,
        {
          project: args.projectId,
          ...
          deleteContentsOnDestroy:
            args.bigqueryOptions.deleteContentsOnDestroy ?? false,
        },
        { parent: this },
      );
      ...
      const sink = this.createSink(
        `${name}-bq-sink`,
        {
          ...
          folderId: args.folderId,
          sinkName:
            args.bigqueryOptions.loggingSinkName ??
            `sk-c-logging-bq-${args.bigqueryOptions.datasetName}`,
      ...
      const iam = new gcp.bigquery.DatasetIamMember(
        `${name}-bq-iam`,
        {
          project: args.projectId,
    ...
    if (args.storageOptions?.bucketName) {
        ...
        `${name}-storage-bucket`,
        {
          ...
          retentionPolicy: args.storageOptions.retentionPolicy
            ? {
                ...
                retentionPeriod:
                  (args.storageOptions.retentionPolicy.retentionPeriodDays ??
                    365) * 86400,
              }
            : undefined,
        },
      ...
      const sink = this.createSink(
        `${name}-storage-sink`,
        {
          ...
          folderId: args.folderId,
          sinkName:
            args.storageOptions.loggingSinkName ??
            `sk-c-logging-bkt-${args.storageOptions.bucketName}`,
      ...
      const iam = new gcp.storage.BucketIAMMember(
        `${name}-storage-iam`,
        {
          bucket: bucket.name,
    ...
    if (args.pubsubOptions?.topicName) {
      ...
      const sink = this.createSink(
        `${name}-pubsub-sink`,
        {
          ...
          folderId: args.folderId,
          sinkName:
            args.pubsubOptions.loggingSinkName ??
            `sk-c-logging-pub-${args.pubsubOptions.topicName}`,
      ...
      const iam = new gcp.pubsub.TopicIAMMember(
        `${name}-pubsub-iam`,
        {
          project: args.projectId,
          topic: topic.name,
    ...
    this.waitIamMembership = pulumi
      .all(iamDependencies.map((iam) => iam.id))
      .apply(() => true);
