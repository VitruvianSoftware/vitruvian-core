...
export class VpcServiceControls extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.accessLevel = new gcp.accesscontextmanager.AccessLevel(
      `${name}-al`,
      {
    ...
    // ====================================================================
    const membersDryRun =
      args.membersDryRun && args.membersDryRun.length > 0
        ? args.membersDryRun
        : args.members;
    ...
    this.accessLevelDryRun = new gcp.accesscontextmanager.AccessLevel(
      `${name}-al-dry`,
      {
        parent: normalizedPolicy,
        name: alDryRunName,
    ...
    const resources = pulumi
      .all(args.projectNumbers)
      .apply((nums) => nums.map((p) => `projects/${p}`));
    const restrictedServices =
      args.restrictedServices && args.restrictedServices.length > 0
        ? args.restrictedServices
        : DEFAULT_RESTRICTED_SERVICES;
    const restrictedServicesDryRun =
      args.restrictedServicesDryRun && args.restrictedServicesDryRun.length > 0
    ...
    const status:
      gcp.types.input.accesscontextmanager.ServicePerimeterStatus | undefined =
      args.enforce
        ? {
            resources,
    ...
    this.perimeter = new gcp.accesscontextmanager.ServicePerimeter(
      `${name}-perim`,
      {
        parent: normalizedPolicy,
        name: perimName,
