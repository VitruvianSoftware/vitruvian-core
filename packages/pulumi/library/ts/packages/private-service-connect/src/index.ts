...
export class PrivateServiceConnect extends pulumi.ComponentResource {
  constructor(
    ...
    const addr = new gcp.compute.GlobalAddress(
      `${name}-psc-addr`,
      {
    ...
    if (args.serviceDirectoryNamespace || args.serviceDirectoryRegion) {
      ...
      if (args.serviceDirectoryNamespace) {
        fwdArgs.serviceDirectoryRegistrations.namespace =
          args.serviceDirectoryNamespace;
      }
      if (args.serviceDirectoryRegion) {
        fwdArgs.serviceDirectoryRegistrations.serviceDirectoryRegion =
          args.serviceDirectoryRegion;
      }
    }

    new gcp.compute.GlobalForwardingRule(`${name}-psc-fwd-rule`, fwdArgs, {
      parent: this,
    });
