...
export class InstanceTemplate extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.template = new gcp.compute.InstanceTemplate(
      `${name}-template`,
      {
        ...
        disks: [
          {
            sourceImage:
              args.sourceImage ??
              `projects/${args.sourceImageProject ?? "debian-cloud"}/global/images/family/${args.sourceImageFamily ?? "debian-12"}`,
...
export class ComputeInstance extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    for (let i = 0; i < numInstances; i++) {
      ...
      const instance = new gcp.compute.InstanceFromTemplate(
        `${name}-instance${instanceSuffix}`,
        {
