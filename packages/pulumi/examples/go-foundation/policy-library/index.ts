...
const foundationPolicies = new policy.PolicyPack("foundation-policies", {
  policies: [
    // --- Project Policies ---
    {
      ...
      description:
        "Projects must disable auto-creation of the default network.",
    ...
    },
    {
      ...
      description:
        "Projects must have 'environment' and 'application_name' labels.",
      enforcementLevel: "mandatory",
      validateResource: (args, reportViolation) => {
        if (args.type === "gcp:organizations/project:Project") {
          ...
          if (!labels["application_name"]) {
            reportViolation(
              "Project is missing required label: 'application_name'",
            );
          }
        }
      },
    ...
    // --- IAM Policies ---
    {
      ...
      description:
        "Service account keys must not be created. Use Workload Identity Federation instead.",
      enforcementLevel: "mandatory",
      validateResource: (args, reportViolation) => {
    ...
    },
    {
      ...
      description:
        "Subnets should have VPC Flow Logs enabled for network monitoring.",
      enforcementLevel: "mandatory",
      validateResource: (args, reportViolation) => {
        if (args.type === "gcp:compute/subnetwork:Subnetwork") {
    ...
    },
    {
      ...
      description:
        "Certain core infrastructure must have deletion protection enabled.",
      enforcementLevel: "mandatory",
      validateResource: (args, reportViolation) => {
        ...
        if (protectedTypes.includes(args.type)) {
          ...
          if (
            args.type === "gcp:organizations/project:Project" &&
            args.props.deletionPolicy !== "PREVENT"
          ) {
            reportViolation("Projects must set deletionPolicy to PREVENT.");
          }
          if (
            args.type === "gcp:kms/cryptoKey:CryptoKey" &&
            args.props.destroyScheduledDuration == null
          ) {
    ...
    },
    {
      ...
      validateResource: (args, reportViolation) => {
        if (
          args.type === "gcp:appengine/flexibleAppVersion:FlexibleAppVersion"
        ) {
          ...
          if (dnssec && dnssec.defaultKeySpecs) {
            for (const spec of dnssec.defaultKeySpecs) {
              if (spec.algorithm === "rsasha1") {
                reportViolation(
                  "DNSSEC must not use the insecure RSASHA1 algorithm.",
                );
              }
            }
          }
