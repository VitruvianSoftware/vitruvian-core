...
// project, mirroring upstream 3-networks-svpc/envs/nonproduction.
...
func TestNetConfigDefaultsReal(t *testing.T) {
	os.Setenv("PULUMI_CONFIG", `{"project:project_id":"prj-n-svpc"}`)
	defer os.Unsetenv("PULUMI_CONFIG")

	err := pulumi.RunErr(func(ctx *pulumi.Context) error {
		cfg := loadNetConfig(ctx)

		assert.Equal(t, "prj-n-svpc", cfg.ProjectID)
		// Default values
