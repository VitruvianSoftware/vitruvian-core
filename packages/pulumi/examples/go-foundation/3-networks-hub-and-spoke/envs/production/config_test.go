...
// 3-networks-hub-and-spoke/envs/production.
func TestPinnedEnvIdentity(t *testing.T) {
	...
	assert.Equal(t, "10.8.192.0/18", spokeSubnet1Cidr)
	assert.Equal(t, "10.9.192.0/18", spokeSubnet2Cidr)
	assert.Equal(t, "10.26.6.0/23", spokeProxy1Cidr)
	assert.Equal(t, "10.27.6.0/23", spokeProxy2Cidr)

	// Secondary ranges only on R1 (matching upstream)
	assert.Equal(t, "100.72.192.0/18", spokeGkePod1Cidr)
	assert.Equal(t, "100.73.192.0/18", spokeGkeSvc1Cidr)
}

// TestLoadNetConfig validates defaults match TF upstream.
func TestLoadNetConfig(t *testing.T) {
	os.Setenv("PULUMI_CONFIG", `{"project:spoke_project_id":"prj-p-svpc"}`)
	defer os.Unsetenv("PULUMI_CONFIG")

	err := pulumi.RunErr(func(ctx *pulumi.Context) error {
		cfg := loadNetConfig(ctx)

		assert.Equal(t, "prj-p-svpc", cfg.SpokeProjectID)
