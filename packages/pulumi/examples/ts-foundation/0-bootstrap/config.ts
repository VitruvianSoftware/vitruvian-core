...
export function loadConfig(pulumiConfig: pulumi.Config): BootstrapConfig {
  ...
  return {
    ...
    kmsKeyProtectionLevel:
      pulumiConfig.get("kms_key_protection_level") || "SOFTWARE",
    projectDeletionPolicy:
      pulumiConfig.get("project_deletion_policy") || "PREVENT",
    parentFolder,
    parent,
    parentType,
    parentId,
    ...
    orgPolicyAdminRole:
      pulumiConfig.getBoolean("org_policy_admin_role") ?? false,
    bucketForceDestroy:
      pulumiConfig.getBoolean("bucket_force_destroy") ?? false,
    bucketTfStateKmsForceDestroy:
      pulumiConfig.getBoolean("bucket_tfstate_kms_force_destroy") ?? false,
    folderDeletionProtection:
      pulumiConfig.getBoolean("folder_deletion_protection") ?? true,
    workflowDeletionProtection:
      pulumiConfig.getBoolean("workflow_deletion_protection") ?? true,
    randomSuffix: pulumiConfig.getBoolean("random_suffix") ?? true,

    groups: {
      createRequiredGroups:
        pulumiConfig.getBoolean("create_required_groups") ?? false,
      createOptionalGroups:
        pulumiConfig.getBoolean("create_optional_groups") ?? false,
      ...
      optionalGroups: {
        ...
        gcpGlobalSecretsAdmin:
          pulumiConfig.get("gcp_global_secrets_admin") || "",
        gcpKmsAdmin: pulumiConfig.get("gcp_kms_admin") || "",
      },
    },
    initialGroupConfig:
      pulumiConfig.get("initial_group_config") || "WITH_INITIAL_OWNER",
