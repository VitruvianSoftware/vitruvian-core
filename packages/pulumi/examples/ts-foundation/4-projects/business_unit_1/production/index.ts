...
export = async () => {
  ...
  const businessUnitName =
    config.get("business_unit_name") || "business_unit_1";
  const envCode = "p";
  ...
  const netRef = new pulumi.StackReference(`net-svpc-production`);
  const sharedVpcHostProjectId = orgRef
    .getOutput("shared_vpc_projects")
    .apply(
      (p: Record<string, { project_id: string }>) =>
        p["production"]?.project_id || "",
    );

  // BU folder under env
  const buFolder = new gcp.organizations.Folder("bu-folder", {
    displayName: `${folderPrefix}-production-${businessCode}`,
    parent: envFolder,
  ...
  const firewallEnableLogging =
    config.getBoolean("firewall_enable_logging") ?? true;
  const peeringIapFwEnabled =
    config.getBoolean("peering_iap_fw_rules_enabled") ?? true;
  const optionalFwRulesEnabled =
    config.getBoolean("optional_fw_rules_enabled") ?? false;
  const windowsActivationEnabled =
    config.getBoolean("windows_activation_enabled") ?? false;
  ...
  const hostNetworkSelfLink = netRef.getOutput(
    "network_self_link",
  ...
  const peeringRules: any[] = [
    {
      ruleName: `fw-${envCode}-peering-base-65530-e-p-all-all-tcp-udp`,
    ...
    },
    {
      ...
      description:
        "Lower priority rule to allow private google apis on TCP port 443.",
      direction: "EGRESS",
      action: "allow",
      priority: 10000,
  ...
  const perimeterName = netRef.getOutput(
  ...
  new gcp.accesscontextmanager.ServicePerimeterResource(
    `production-confidential-space-perimeter-attachment`,
    {
  ...
  const workloadIdentitySa = new gcp.serviceaccount.Account(
    `production-confidential-space-sa`,
    {
  ...
  const cmekKeyRing = new gcp.kms.KeyRing(`production-cmek-keyring`, {
    name: `${projectPrefix}-p-${businessCode}-cmek-keyring`,
    location: config.get("default_region") || "us-central1",
    project: cmekProject.projectId,
  });

  const cmekKey = new gcp.kms.CryptoKey(`production-cmek-key`, {
    name: `${projectPrefix}-p-${businessCode}-cmek-key`,
  ...
  const storageSa = cmekProject.projectNumber.apply(
    (num) =>
      `serviceAccount:service-${num}@gs-project-accounts.iam.gserviceaccount.com`,
  );
  new gcp.kms.CryptoKeyIAMMember(`production-cmek-key-binding`, {
  ...
  let iapFirewallTags: pulumi.Output<Record<string, string>> = pulumi.output(
  ...
  if (
    ...
    iapFirewallTags = pulumi
      .all([sshTagKey.id, sshTagValue.id, rdpTagKey.id, rdpTagValue.id])
  ...
  return {
    ...
    vpc_service_control_perimeter_name: netRef.getOutput(
      "service_perimeter_name",
    ),
    access_context_manager_policy_id: netRef.getOutput(
