...
export function deployEnvBaseline(args: EnvBaselineArgs): EnvBaselineOutputs {
  ...
  if (args.assuredWorkloadConfiguration?.enabled) {
    const workload = new gcp.assuredworkloads.Workload(
      `${name}-assured-workload`,
      {
        ...
        complianceRegime:
          args.assuredWorkloadConfiguration.complianceRegime ||
          "FEDRAMP_MODERATE",
        displayName:
          args.assuredWorkloadConfiguration.displayName || `${name}-workload`,
        location: args.assuredWorkloadConfiguration.location || "us-central1",
        resourceSettings: [
          {
            resourceType:
              args.assuredWorkloadConfiguration.resourceType ||
              "CONSUMER_FOLDER",
          },
        ],
      },
