...
import {
  OrgPolicyBoolean,
  OrgPolicyList,
  DomainRestrictedSharing,
} from "@vitruviansoftware/foundation-org-policy";
...
export = async () => {
  ...
  const bootstrapStackName =
    new pulumi.Config().get("bootstrap_stack_ref") || "bootstrap";
  const bootstrapRef = new pulumi.StackReference(bootstrapStackName);
  const commonConfig = bootstrapRef.getOutput("common_config") as pulumi.Output<
    Record<string, string>
  >;
  const requiredGroups = bootstrapRef.getOutput(
    "required_groups",
  ) as pulumi.Output<Record<string, string>>;
  const optionalGroups = bootstrapRef.getOutput(
    "optional_groups",
  ) as pulumi.Output<Record<string, string>>;

  const orgId = commonConfig.apply((c) => c.org_id || cfg.orgId);
  const billingAccount = commonConfig.apply(
    (c) => c.billing_account || cfg.billingAccount,
  );
  const projectPrefix = commonConfig.apply(
    (c) => c.project_prefix || cfg.projectPrefix,
  );
  const folderPrefix = commonConfig.apply(
    (c) => c.folder_prefix || cfg.folderPrefix,
  );
  const parent = commonConfig.apply((c) => c.parent_id || cfg.parent);
  const bootstrapFolderName = commonConfig.apply(
    (c) => c.bootstrap_folder_name,
  );
  ...
  if (cfg.essentialContactsDomainsToAllow.length > 0) {
    new OrgPolicyList("essential-contacts-domain-restriction", {
      ...
      policyType: "allow",
      values: cfg.essentialContactsDomainsToAllow.map((d) =>
        d.startsWith("@") ? d : `@${d}`,
      ),
    });
  }
  ...
  const orgAuditLogs = new ProjectFactory("org-audit-logs", {
    ...
    budgetAlertSpentPercents:
      cfg.projectBudget.org_audit_logs_alert_spent_percents,
    budgetAlertSpendBasis:
      cfg.projectBudget.org_audit_logs_budget_alert_spend_basis,
  ...
  const orgBillingExport = new ProjectFactory("org-billing-export", {
    ...
    budgetAlertPubsubTopic:
      cfg.projectBudget.org_billing_export_alert_pubsub_topic,
    budgetAlertSpentPercents:
      cfg.projectBudget.org_billing_export_alert_spent_percents,
    budgetAlertSpendBasis:
      cfg.projectBudget.org_billing_export_budget_alert_spend_basis,
    defaultServiceAccount: "deprivilege",
    activateApis: [
      "logging.googleapis.com",
      "bigquery.googleapis.com",
  ...
  const commonKms = new ProjectFactory("common-kms", {
    ...
    budgetAlertSpendBasis:
      cfg.projectBudget.common_kms_budget_alert_spend_basis,
    defaultServiceAccount: "deprivilege",
    activateApis: [
      "logging.googleapis.com",
  ...
  const orgSecrets = new ProjectFactory("org-secrets", {
    ...
    budgetAlertSpentPercents:
      cfg.projectBudget.org_secrets_alert_spent_percents,
    budgetAlertSpendBasis:
      cfg.projectBudget.org_secrets_budget_alert_spend_basis,
    defaultServiceAccount: "deprivilege",
    activateApis: [
      "logging.googleapis.com",
  ...
  const interconnect = new ProjectFactory("interconnect", {
    ...
    budgetAlertSpentPercents:
      cfg.projectBudget.interconnect_alert_spent_percents,
    budgetAlertSpendBasis:
      cfg.projectBudget.interconnect_budget_alert_spend_basis,
    defaultServiceAccount: "deprivilege",
  ...
  const sccNotifications = new ProjectFactory("scc-notifications", {
    ...
    budgetAlertPubsubTopic:
      cfg.projectBudget.scc_notifications_alert_pubsub_topic,
    budgetAlertSpentPercents:
      cfg.projectBudget.scc_notifications_alert_spent_percents,
    budgetAlertSpendBasis:
      cfg.projectBudget.scc_notifications_budget_alert_spend_basis,
    defaultServiceAccount: "deprivilege",
    activateApis: [
      "logging.googleapis.com",
  ...
  for (const [env, envCode] of Object.entries(environments)) {
    environmentNetworkProjects[env] = new ProjectFactory(`env-network-${env}`, {
      ...
      budgetAlertPubsubTopic:
        cfg.projectBudget.shared_network_alert_pubsub_topic,
      budgetAlertSpentPercents:
        cfg.projectBudget.shared_network_alert_spent_percents,
      budgetAlertSpendBasis:
        cfg.projectBudget.shared_network_budget_alert_spend_basis,
      defaultServiceAccount: "deprivilege",
      activateApis: [
        "compute.googleapis.com",
        "dns.googleapis.com",
...
logName: /logs/dns.googleapis.com%2Fdns_queries`;
  ...
  const centralizedLogging = new CentralizedLogging(
    "org-logging",
    {
      ...
      billingAccount: cfg.enableBillingAccountSink
        ? cfg.billingAccount
        : undefined,
      ...
      storageOptions: {
        bucketName:
          pulumi.interpolate`bkt-${orgAuditLogs.projectId}-org-logs-${logBucketSuffix.result}` as unknown as string,
  ...
  if (cfg.enableSccResources) {
    const sccTopic = new gcp.pubsub.Topic(
      "scc-notification-topic",
      {
    ...
    new gcp.securitycenter.V2OrganizationNotificationConfig(
      "scc-notification",
      {
  ...
  if (cfg.createUniqueTagKey) {
    ...
    tagKeyShortName =
      pulumi.interpolate`environment-${tagKeySuffix.result}` as any;
  }
  ...
  if (cfg.enableCaiMonitoring) {
    ...
    caiMonitoring = new CAIMonitoring(
      "cai-monitoring",
      {
        orgId: cfg.orgId,
  ...
  if (cfg.createAccessContextManagerAccessPolicy) {
    const accessPolicy = new gcp.accesscontextmanager.AccessPolicy(
      "access-policy",
      {
        parent: `organizations/${cfg.orgId}`,
  ...
  return {
    ...
    shared_vpc_projects: Object.fromEntries(
      Object.entries(environmentNetworkProjects).map(([k, v]) => [
        k,
        { project_id: v.projectId, project_number: v.projectNumber },
      ]),
    ),
    tags: {
      key_id: tagKey.id,
      values: Object.fromEntries(
        Object.entries(tagValues).map(([k, v]) => [k, v.id]),
      ),
    },
    ...
    logs_export_project_linked_dataset_name:
      centralizedLogging.linkedDatasetName,
