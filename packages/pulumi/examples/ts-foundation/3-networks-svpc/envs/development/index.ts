...
} from "@vitruviansoftware/foundation-vpc-service-controls";
...
export = async () => {
  ...
  const envProjectNumber = sharedVpcProjects.apply(
    (p) => p["development"]?.project_number || "",
  );
  const additionalVpcScProjects =
    config.getObject<string[]>("vpc_sc_project_numbers") || [];
  ...
  const enableDedicatedInterconnect =
    config.getBoolean("enable_dedicated_interconnect") ?? false;
  ...
  const targetNameServerAddresses = config.getObject<string[]>(
