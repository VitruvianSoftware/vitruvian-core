...
function buildFoundationRules(
  ...
  const rules: PolicyFirewallRule[] = [
    ...
    },
    {
      ...
      description:
        "Lower priority rule to allow restricted google apis on TCP port 443.",
      enableLogging,
      match: {
...
export class SharedVpc extends pulumi.ComponentResource {
  ...
  public readonly subnetsSecondaryRanges: pulumi.Output<
    Record<string, { rangeName: string; ipCidrRange: string }[]>
  >;
  ...
  constructor(
    ...
    const modeStr =
      args.mode === "hub" ? "-hub" : args.mode === "spoke" ? "-spoke" : "";
    ...
    if (args.windowsActivationEnabled) {
      new gcp.compute.Route(
        `${name}-windows-kms-route`,
        {
          ...
          priority: 1000,
          description:
            "Route through IGW to allow Windows KMS activation for GCP.",
        },
        { parent: this },
      );
    }
    ...
    this.subnetsNames = pulumi
      .all(subnetEntries.map((s) => s.name))
      .apply((names) => names);
    this.subnetsIps = pulumi
      .all(subnetEntries.map((s) => s.ipCidrRange))
      .apply((ips) => ips);
    this.subnetsSelfLinks = pulumi
      .all(subnetEntries.map((s) => s.selfLink))
      .apply((links) => links);
    this.subnetsSecondaryRanges = pulumi.output(args.secondaryRanges || {});

    // DNS policy
    const dnsPolicy = new gcp.dns.Policy(
      `${name}-dns-policy`,
      {
        project: args.projectId,
    ...
    for (const [zoneName, dnsName] of [
      ...
      const cnameTarget =
        zoneName === "googleapis" ? "restricted.googleapis.com." : dnsName;
      const aRecordName =
        zoneName === "googleapis" ? "restricted.googleapis.com." : dnsName;

      new gcp.dns.RecordSet(
        `${name}-dns-${zoneName}-cname`,
        {
          project: args.projectId,
    ...
    if (
      args.mode === "spoke" &&
      args.netHubProjectId &&
      args.netHubNetworkSelfLink
    ) {
      const spokeToHub = new gcp.compute.NetworkPeering(
        `${name}-spoke-to-hub`,
        {
      ...
      const hubToSpoke = new gcp.compute.NetworkPeering(
        `${name}-hub-to-spoke`,
        {
    ...
    if (
      args.environmentCode === "p" &&
      args.domain &&
      args.targetNameServerAddresses
    ) {
      new gcp.dns.ManagedZone(
        `${name}-dns-forwarding`,
        {
          ...
          forwardingConfig: {
            targetNameServers: args.targetNameServerAddresses.map(
              (ip: string) => ({
                ipv4Address: ip,
              }),
            ),
    ...
    if (args.natEnabled !== false) {
      ...
      regions.forEach((reg) => {
        const natRouter = new CloudRouter(
          `${name}-nat-router-${reg}`,
          {
            project: args.projectId,
    ...
    const fwPolicy = new gcp.compute.NetworkFirewallPolicy(
      `${name}-fw-policy`,
      {
        project: args.projectId,
        name: policyName,
    ...
    for (const rule of fwRules) {
      ...
      new gcp.compute.NetworkFirewallPolicyRule(
        `${name}-fw-rule-${rule.priority}`,
        {
          project: args.projectId,
          firewallPolicy: fwPolicy.name,
