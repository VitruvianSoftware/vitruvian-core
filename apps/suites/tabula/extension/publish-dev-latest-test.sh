...
#   2. the published bundle's build-info.json is actually re-stamped with the
#      real commit (not the hermetic placeholder).
#   3. both assets are uploaded with --clobber to the rolling tag.
set -euo pipefail

SCRIPT="$TEST_SRCDIR/_main/apps/suites/tabula/extension/publish-dev-latest.sh"
FAKE_ROOT="$TEST_TMPDIR/workspace"
STUBS="$TEST_TMPDIR/stubs"
WORK="$TEST_TMPDIR/runner-temp"
GH_LOG="$TEST_TMPDIR/gh.log"
mkdir -p "$FAKE_ROOT/apps/suites/tabula/extension" "$FAKE_ROOT/bazel-bin/apps/suites/tabula/extension" "$STUBS" "$WORK"

# --- fake workspace ---------------------------------------------------------
printf '{"version":"9.9.9-test"}\n' >"$FAKE_ROOT/apps/suites/tabula/extension/package.json"
# A real zip whose build-info.json is the hermetic placeholder, as webpack ships it.
(cd "$TEST_TMPDIR" &&
	printf '{"commit":"PLACEHOLDER"}\n' >build-info.json &&
	zip -q "$FAKE_ROOT/bazel-bin/apps/suites/tabula/extension/tabula-extension-chrome.zip" build-info.json &&
	rm build-info.json)
...
unzip -p "$WORK/tabula-extension-chrome.zip" build-info.json >"$TEST_TMPDIR/stamped.json"
grep -q 'deadbeefcafe' "$TEST_TMPDIR/stamped.json" || fail "bundle not re-stamped: $(cat "$TEST_TMPDIR/stamped.json")"
grep -q 'PLACEHOLDER' "$TEST_TMPDIR/stamped.json" && fail "placeholder survived in build-info.json"

# The standalone identity asset is published from $WORK by absolute path.
[ -f "$WORK/build-info.json" ] || fail "standalone build-info.json missing from \$WORK"
