...
[`apps/suites/tabula/infra`](../../infra). It is driven exclusively through the monorepo's Bazel
...
```mermaid
flowchart LR
    subgraph tabula_infra["apps/suites/tabula/infra (Pulumi-in-Go)"]
...
| Stack | Path | Manages |
|---|---|---|
| **identity** | [`apps/suites/tabula/infra/identity`](../../infra/identity) | Repo-scoped **Workload Identity Federation** pool/provider, a least-privilege **deploy** service account, and the **runtime** service account — keyless, no JSON keys |
| **build** | [`apps/suites/tabula/infra/build`](../../infra/build) | The Artifact Registry repository the API image is pushed to |
| **data** | [`apps/suites/tabula/infra/data`](../../infra/data) | The stateful tier as code — **Neon** Postgres and **Upstash** Redis (provisioned by Pulumi, not clicked in a console) |
| **app** | [`apps/suites/tabula/infra/app`](../../infra/app) | The **Cloud Run v2** service, traffic/revisions, runtime env + secret wiring, and the custom domain mapping |

Each stack exposes the standard wrapper verbs (identity injected per-project from
`infrastructure/gcp-identities.tsv`):

```bash
bazel run //apps/suites/tabula/infra/<stack>:preview     # everyday: see the diff
bazel run //apps/suites/tabula/infra/<stack>:up          # break-glass only — the pipeline applies
...
The local development `.env` (`apps/suites/tabula/api/.env`) is a **separate, gitignored** set of
