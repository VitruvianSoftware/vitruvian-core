...
**pnpm** (via Corepack), and **Go** (for the `apps/suites/tabula/infra` Pulumi programs). For cloud
work you also need **gcloud** and **gh**.

```bash
# from the repo root
nvm use && corepack enable     # Node 22 + pnpm (versions come from .nvmrc / packageManager)
bazel run //apps/suites/tabula:doctor      # verify your toolchain has exactly what Tabula needs
```

`//apps/suites/tabula:doctor` checks Tabula's specific requirements and fails with a clear message
...
The API reads a **gitignored** `apps/suites/tabula/api/.env`. Create it from the committed
template and fill in the values:

```bash
cp apps/suites/tabula/api/.env.example apps/suites/tabula/api/.env
```
