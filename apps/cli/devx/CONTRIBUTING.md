...
4. **The monorepo imports your PR automatically** (within ~15 min — a scheduled job), opening a PR in `vitruvian-core` under `apps/cli/devx/` **with you as the author**, where the full monorepo CI + review run.
...
> **Note:** the older "fork → PR → squash-merge here" and AI-agent merge steps below describe the legacy standalone workflow and are retained only for local build/dev reference. **Merges happen in the monorepo now, not in this mirror.** Keep changes scoped to the `apps/cli/devx/` subtree.
