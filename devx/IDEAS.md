...
### 65. Bootable Cluster-Join Media (`devx cluster iso`)
* **Priority:** 🟢 P1
* **Effort:** High
* **Impact:** Turns any spare machine — old laptop, NUC, bare-metal box — into a cluster node by booting a USB stick. No SSH, no Lima, no same-LAN requirement; a booted node joins from any network via Tailscale.
* **The Problem:** Today `devx cluster init/join` models every node as a macOS host that devx reaches over SSH and drives `limactl` on to run a Lima VM (`internal/multinode/remote/remote.go` → `internal/multinode/lima`). That's great for a fleet of Macs, but adding a node is a push-based, SSH-dependent operation: the operator's machine must have SSH access to the node, the node must be a Lima-capable host, and (outside the Tailscale path) it must sit on the same LAN. There's no way to take an arbitrary spare machine — especially one on a different network — and have it join unattended just by booting it.
* **The Solution:** Add `devx cluster iso` — generate a bootable, self-installing image that turns any x86_64/arm64 machine into a cluster node on first boot, with zero manual provisioning. The command reads `cluster.yaml`, pulls the live join token from a running server (`/var/lib/rancher/k3s/server/node-token`, exactly as `cluster.Join` already does), and templates a Fedora CoreOS Butane config with three ordered systemd units: bring up Tailscale (`tailscale up --authkey`), wait for `tailscale0` to get an IP, then run the *same* k3s agent installer devx already uses (`curl -sfL https://get.k3s.io | K3S_URL=https://<server-tailscale-ip>:6443 K3S_TOKEN=… sh -s - agent --flannel-iface=tailscale0`). It compiles the Butane via the existing `internal/ignition` package and wraps `coreos-installer iso customize --dest-ignition` to emit a flashable `.iso`. Because the join rides the tailnet, a booted node joins from any network/NAT — matching devx's existing Tailscale cluster networking, where the API server already advertises its Tailscale IP as a TLS SAN (`cluster.go` Phase 1.5 + `InitCluster` `--tls-san`).
  * **Reuses existing primitives:** the k3s agent join (`k3s.go:JoinAgent`), Tailscale bring-up (`k3s.go:InstallTailscale`), token retrieval (`k3s.go:GetToken`), and the Butane→Ignition compiler (`internal/ignition/builder.go`). The new code is mostly templating + a `coreos-installer` wrapper, so the core is tractable; the "High" effort is productionization (arch matrix, offline staging, secret handling, lifecycle).
  * **Flags:** `--role server|agent` (default `agent`), `--arch amd64|arm64`, `--pool <label>`, `--k3s-version` (defaults to `cluster.yaml`), `--output node.iso`, `--offline` (pre-stage the k3s binary + images for air-gapped boot instead of `curl | sh`).
  * **Security:** the image embeds the cluster node-token and a Tailscale auth key, so it is a secret artifact — anyone who boots it joins the cluster. Recommend reusable, ephemeral-off, pre-approved, tag-limited Tailscale keys (per `cluster.yaml.example`) and document the blast radius.
  * **Caveats to design around:** ISO arch must match target hardware; k3s version should match the cluster; and a USB-joined node currently lives *outside* devx's Lima-on-SSH lifecycle model — `cluster status/upgrade/remove` won't manage it (it shows up only in `kubectl get nodes`). A follow-up could let `cluster.yaml` register "external" nodes so the lifecycle commands see them.
  * **Validate before building:** prove an off-LAN Tailscale agent join by hand, then boot a `coreos-installer`-customized ISO in a local hypervisor and confirm auto-join, *before* writing the Go command. Both spikes use off-the-shelf tooling and no devx code.
* **Key files:** `cmd/cluster_mgmt_iso.go` (new), `internal/multinode/iso/builder.go` (new), `cluster-node.template.bu` (new Butane template), `internal/ignition/builder.go` (reuse compiler), `internal/multinode/k3s/k3s.go` (extract the agent-join command into a shared string builder so CLI and ISO emit identical commands).
